...
def save_chat_to_history(query, result):
...
def load_chat_from_history(chat_id):
    ...
    for chat in st.session_state.chat_history:
        if chat['id'] == chat_id:
            ...
            st.session_state.crew_result = chat['result']
...
def setup_crew():
    ...
    agent_3 = Agent(
        ...
        backstory="You are a data visualization expert who writes clear, production-ready Python scripts using seaborn and matplotlib. Your scripts analyze DataFrame columns to choose suitable plots (bar, line, scatter, heatmap, etc.), set proper titles, labels, legends, and save plots as high-quality images. You ensure the output is ONLY the code for diagram.py with no explanations or formatting. Always suppress warnings and avoid deprecated parameters.",
        llm=llm
    )

    task_3 = Task(
        description="Take the pandas DataFrame output from agent_2 and produce ONLY the raw contents of a valid diagram.py file. The file should contain complete, executable Python code that uses seaborn and matplotlib to create appropriate plots based on the DataFrame's columns. Include clear titles, axis labels, legends, and save the plots as high-quality images. IMPORTANT: Add import warnings; warnings.filterwarnings('ignore') at the top to suppress all warnings. Use current best practices and avoid deprecated parameters. Do not add any hyperparameters or advanced configuration. Keep plots simple and clean. Output ONLY the code itself with no additional text, no markdown formatting (like ```python), and no quotes at the start or end. The code must be ready to save and run as a .py file without modification.",
        expected_output="The exact contents of a valid Python script (diagram.py) containing only executable seaborn and matplotlib code to produce professional visualizations from the DataFrame and save them as images, with warnings suppressed and no deprecated parameters, with no extra text or formatting.",
        agent=agent_3,
...
def main():
    ...
    with st.sidebar:
        ...
        # New Chat button
        if st.button("🆕 New Chat", key="new_chat", help="Start a new chat session"):
            create_new_chat()
            ...
            for i, chat in enumerate(st.session_state.chat_history):
                ...
                with col1:
                    if st.button(
                        f"📝 {chat['preview']}", 
                        key=f"chat_{chat['id']}", 
                        help=f"Load chat from {chat['timestamp']}"
                    ):
                        load_chat_from_history(chat['id'])
                
                with col2:
                    if st.button("🗑️", key=f"delete_{chat['id']}", help="Delete this chat"):
                        delete_chat_from_history(chat['id'])
                
                st.markdown(f"<small style='color: #6b7280;'>{chat['timestamp']}</small>", unsafe_allow_html=True)
                st.markdown("---")
    ...
    # Query input - use active_query if available
    query = st.text_area(
        ...
        if not st.session_state.analysis_complete or st.session_state.get('last_query') != query:
            ...
            with st.spinner("Analyzing query and generating insights..."):
                try:
                    ...
                    save_chat_to_history(query, result)
